...
ed_config = EditDistance(insertion_cost=1, deletion_cost=1, substitution_cost=2)


def agreement_score(gold_span,
                    predicted_span,
                    edit_distance=ed_config):
    """
    ...
    denominator = (edit_distance._substitution_cost - 1) * min(len_gold, len_target) + \ max(len_gold, len_target)

    return 1 - (distance / denominator)


def exact_match_score(gold_span, predicted_span):
    """
...
def tp_fp_fn_tn_role_agreement_single_gold(gold_span: str,
    ...
    if gold_span != "" and predicted_span != "":
        # compute tp for all roles present in predicted_dict
        max_tp_score = agreement_fn(gold_span, predicted_span)
        tp += max_tp_score
        count_tp_instances += 1
