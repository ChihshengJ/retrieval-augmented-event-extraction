# import sys
# sys.path.append("/data/cjin/retrieval-augmented-event-extraction/src")
import json
import numpy as np
...
import src.iterx.metrics.famus.iterx_famus
...
def get_compute_ceafe_rme_scores(
    gold_file,
    predictions,
    ...
    iterx_famus(predictions, gold_file, normalize_role=False)
...
def get_results_based_on_threshold_qa(results, threshold=0.02):
    """
...
def return_cdae_qa_predictions(split,
    ...
    with open(f"../../models/cdae/best_model_{context}_qa/results_{split}.json", 'r') as f:
        results = json.load(f)
    with open(f"../../data/cross_doc_role_extraction/qa_format/{context}_data/{split}.json", 'r') as f:
        gold = [json.loads(line) for line in f.readlines()]

    predictions_qa = aggregate_predictions_for_QA([x['id'] for x in gold], get_highest_score_results_qa(results))
